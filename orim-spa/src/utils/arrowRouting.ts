...
function computeOrthogonalRoute(
  ...
  const startStub = offsetPoint(start, sourceDock, ORTHOGONAL_DOCK_STUB_LENGTH);
  const endStub = offsetPoint(end, targetDock, ORTHOGONAL_DOCK_STUB_LENGTH);
  ...
  const isHorizontalSource = sourceDock === DockPoint.Left || sourceDock === DockPoint.Right;
  const isHorizontalTarget = targetDock === DockPoint.Left || targetDock === DockPoint.Right;
...
function findOrthogonalPathAvoidingObstacles(
  ...
  const sourceHorizontal = sourceDock === DockPoint.Left || sourceDock === DockPoint.Right;
  const targetHorizontal = targetDock === DockPoint.Left || targetDock === DockPoint.Right;
