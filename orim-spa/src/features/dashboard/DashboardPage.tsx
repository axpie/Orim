...
export function DashboardPage() {
  ...
  const defaultBoardThemeKey = useMemo(
    () => themes.some((theme) => theme.isEnabled && theme.key === userThemeKey) ? userThemeKey : '',
    [themes, userThemeKey],
  );
  ...
  const handleCreate = () => {
    ...
    setNewTitle('');
    setNewTemplate('');
    setNewThemeKey(defaultBoardThemeKey);
  ...
  const createBoardWithPreset = (templateId?: string) => {
    ...
    createMutation.mutate({
      ...
      templateId,
      themeKey: defaultBoardThemeKey || undefined,
      visibility: BoardVisibility.Private,
    });
    markOnboardingSeen();
  };

  const openCreateDialog = (options?: { title?: string; templateId?: string }) => {
    setNewTitle(options?.title ?? '');
    setNewTemplate(options?.templateId ?? '');
    setNewThemeKey(defaultBoardThemeKey);
    setCreateOpen(true);
  };
                ...
                <Grid size={{ xs: 6, sm: 4, md: 2 }} key={tmpl.id}>
                  <Card variant="outlined" sx={{ height: '100%', textAlign: 'left' }}>
                    <CardActionArea
                      ...
                      onClick={() => openCreateDialog({ templateId: tmpl.id, title: getTemplateTitle(tmpl) })}
                    >
