...
describe('boardStore', () => {
  ...
  describe('comments', () => {
    it('should set comments', () => {
      useBoardStore.getState().setBoard(createTestBoard());
      const comment = { id: 'c1', boardId: 'test-board-id', authorUserId: 'u1', authorUsername: 'user', x: 0, y: 0, text: 'hi', replies: [], createdAt: '', updatedAt: '' };
      useBoardStore.getState().setComments([comment]);
      expect(useBoardStore.getState().board!.comments).toHaveLength(1);
    });

    it('should upsert an existing comment', () => {
      const comment = { id: 'c1', boardId: 'test-board-id', authorUserId: 'u1', authorUsername: 'user', x: 0, y: 0, text: 'hi', replies: [], createdAt: '', updatedAt: '' };
      useBoardStore.getState().setBoard(createTestBoard());
      useBoardStore.getState().setComments([comment]);
      useBoardStore.getState().upsertComment({ ...comment, text: 'updated' });
      expect(useBoardStore.getState().board!.comments).toHaveLength(1);
      expect(useBoardStore.getState().board!.comments[0].text).toBe('updated');
    });
    ...
    it('should remove a comment', () => {
      ...
      useBoardStore.getState().setBoard(createTestBoard());
      useBoardStore.getState().setComments([comment]);
      useBoardStore.getState().removeComment('c1');
      expect(useBoardStore.getState().board!.comments).toHaveLength(0);
    });
  });
