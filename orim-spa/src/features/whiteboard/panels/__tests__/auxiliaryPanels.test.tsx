...
describe('auxiliaryPanels', () => {
  it('returns the configured width for each panel', () => {
    ...
    expect(getAuxiliaryPanelWidth('comments')).toBe(420);
  ...
  it('toggles the same panel off and switches to a different panel', () => {
    expect(toggleAuxiliaryPanel(null, 'comments')).toBe('comments');
    expect(toggleAuxiliaryPanel('comments', 'comments')).toBeNull();
    expect(toggleAuxiliaryPanel('comments', 'properties')).toBe('properties');
  });
});
